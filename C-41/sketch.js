...
function preload() {
    ...
    imageOver = loadImage('images/gameOver.jpg');
    incom_car1_img = loadImage('images/incoming_car.png');
}
function setup() {
    canvas = createCanvas(displayWidth, displayHeight);
    incom_cars_group = new Group();
...
function draw() {
    ...
    image(background_img, 0, -displayHeight * 5, displayWidth, displayHeight * 6);
    drawSprites();
    ...
    if(incom_cars_group.bounceOff(car)){
        lifes= lifes-1
    }
    if(lifes === 5){
        life1.visible = true;
...
function imcom_cars() {
    if (frameCount % 10 === 0) {
        ...
        incom_cars_group.add(incom_car1);
    }
}
